  ...
  generateDirectory = pkgs.writeScriptBin "generate-directory" ''
    if [ $# -eq 0 ]
      then
        ...
        cp ${jupyter}/lib/python3.8/site-packages/jupyterlab/staging/yarn.lock "$DIRECTORY"/staging
  ...
  mkDirectoryFromLockFile = { yarnlock, packagefile, extensions ? [], sha256 }:
    ...
    pkgs.stdenv.mkDerivation {
      ...
      installPhase = ''
        ...
        mkdir -p $FOLDER
        cp -R ${jupyter}/lib/python3.8/site-packages/jupyterlab/* $FOLDER
        chmod -R +rw $FOLDER
  ...
  mkDirectoryWith = { extensions }:
    ...
    pkgs.stdenv.mkDerivation {
      ...
      installPhase = ''
        ...
        cp ${jupyter}/lib/python3.8/site-packages/jupyterlab/staging/yarn.lock appdir/staging
